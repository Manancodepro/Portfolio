...
export default function Contact() {
    ...
    const validate = () => {
        ...
        else if (!/^\S+@\S+\.\S+$/.test(form.email)) { e.email = 'Invalid email address'; ok = false }
        if (!form.message.trim()) { e.message = 'Message is required'; ok = false }
        else if (form.message.trim().length < 10) { e.message = 'Message must be at least 10 characters'; ok = false }
        setErrors(e)
        return ok
    }
    ...
    const showStatus = (type) => {
        ...
        requestAnimationFrame(() => {
            ...
            gsap.fromTo(statusRef.current, { y: 10, opacity: 0 }, { y: 0, opacity: 1, duration: 0.4 })
        })
        setTimeout(() => {
            gsap.to(statusRef.current, { opacity: 0, y: -10, duration: 0.4, onComplete: () => setStatus(null) })
        }, 5000)
    }
                ...
                <div className="contact-wrapper">
                    ...
                    <div className="contact-info">
                        ...
                        <p>
                            ...
                            I'd love to hear from you. Let's build something extraordinary together.
                        </p>
